        ...
        body.innerHTML += `
            <tr>
                ...
                <div class="d-flex justify-content-center gap-1">
                    <button class="btn btn-sm btn-light border" onclick="detailPembelian(${
                        item.id
                    })" title="Detail">
                    ...
                    <button class="btn btn-sm btn-light border" onclick="deletePembelian(${
                        item.id
                    })" title="Hapus">
        ...
        if (barangList && barangList.length > 0) {
            barangList.forEach((barang) => {
                dropdown.innerHTML += `<option value="${
                    ...
                    barang.harga_jual || barang.harga
...
function tambahItemBaru(skipLoad = false) {
    ...
    if (globalBarangList && globalBarangList.length > 0) {
        ...
        globalBarangList.forEach((barang) => {
            select.innerHTML += `<option value="${barang.id}" data-harga="${
                barang.harga_jual || barang.harga
            }" data-stok="${barang.jumlah}">${barang.nama_barang}</option>`;
        });
    }
...
async function editPembelian(id) {
    ...
    try {
        ...
        document.getElementById("inputPembelianModalLabel").innerHTML =
            '<i class="mdi mdi-pencil"></i> Edit Pembelian';
        ...
        data.items.forEach((item, index) => {
            ...
            barangList.forEach((b) => {
                select.innerHTML += `<option value="${b.id}" data-harga="${
                    b.harga_jual || b.harga
