...
function tambahItemBaru(skipLoad = false) {
    ...
    if (globalBarangList && globalBarangList.length > 0) {
        ...
        globalBarangList.forEach(barang => {
            select.innerHTML += `<option value="${barang.id}" data-harga="${barang.harga_jual || barang.harga}" data-stok="${barang.jumlah}">${barang.nama_barang}</option>`;
        });
    }
...
function simpanPesanan() {
    ...
    if (!statusPembayaranEl || !statusPengirimanEl) {
        Swal.fire('Error', 'Harap pilih Status Pembayaran dan Status Pengiriman!', 'error');
        return;
    }
    ...
    try {
        ...
        if (data.status_pembayaran === 'cicilan') {
            ...
            if (cicilans.length > 0) {
                ...
                listBody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Belum ada jadwal cicilan.</td></tr>';
            }
